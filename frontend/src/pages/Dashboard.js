...
import { DollarSign, TrendingDown, Wallet, Shield, AlertCircle, Calendar } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchDashboard = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get(`/dashboard?month=${currentMonth}&year=${currentYear}`);
  ...
  const handleMarkAsPaid = async (paymentMethod) => {
    try {
      const endpoint = paymentModal.expense.type === 'fixed' 
    ...
    <div className="space-y-6" data-testid="dashboard-page">
      ...
      {dashboard?.alerts?.length > 0 && (
        <div className="space-y-2">
          {dashboard.alerts.map((alert, index) => (
            <Alert key={index} className="border-amber-200 bg-amber-50" data-testid="alert-item">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          subtitle={`Reserva: ${formatCurrency(dashboard?.emergency_reserve || 0)}`}
          color="violet"
        />
      </div>
                    ...
                    <tr key={expense.id} className="border-b border-slate-100 hover:bg-slate-50">
                      <td className="py-3 px-4">
                        ...
                        <div className="text-xs text-slate-500">
                          {expense.type === 'fixed' ? 'Gasto Fixo' : 'Gasto Variável'}
                        </div>
                      </td>
                ...
                <div key={category.category_id} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="font-semibold text-slate-900">{formatCurrency(category.amount)}</div>
